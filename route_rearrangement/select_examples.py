...
The best first test cases are linear routes with many *disconnections between steps
that build on each other only incidentally*: a large number of valid orderings
(``ScheduleLattice.count``) and many commutable pairs, i.e. modular rather than
strictly-coupled construction.
...
def rank_corpus(corpus: str, *, limit: int = 0, min_steps: int = 3, max_steps: int = 10,
                progress: bool = True):
    """Yield candidate dicts (unranked) for every qualifying linear route."""
...
def main(argv=None) -> int:
    ...
    print(f"{len(candidates)} qualifying linear routes; top {len(top)}:")
